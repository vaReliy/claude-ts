...
- A reviewer-shaped prompt asking it to assess, independently of the structural pass:
  ...
  - **Live-data probe** — when the change touches `cts-sync.sh` or any code that writes to consumer working trees, require a run of the changed engine against a scratch copy (`cp -a`) of a real consumer before closing the gate; synthetic fixtures and diff review do not satisfy this. Overwrite semantics removed the merge/baseline bug class entirely, but the engine still has real hazards worth probing against live data: the self-update-first re-exec (does it actually complete a full sync after replacing itself, against a real consumer's tree?), the settings.json deep-merge (does a real consumer's existing settings.json — with its own extra keys and array entries — come out intact?), and the ownership/override-rot detectors (do they fire, or stay silent, against a real consumer's actual customization layout?). If no real consumer checkout is reachable, say so explicitly in the report instead of silently downgrading to fixtures.
