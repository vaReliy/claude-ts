...
- **`.claude/scripts/cts-sync.sh` `baseline_audit()`**: new pre-sync audit (update mode) that detects "phantom baselines" — content the consumer's `.cts-version` claims was received, which upstream still ships, but which is absent from the working copy (the signature of an earlier mis-resolved sync). Root-caused from penny: its round-4 sync stamped `6cbf33b` while the T0–T3 Tiered Planning Ladder (an ancestor of that stamp) never landed in its tree; every later 3-way merge then treated the gap as a deliberate local deletion and preserved it silently — no engine fix can restore such content via merge, so loud detection is the only defense. Line-set comparison on prettier-normalized content, ≥3-line threshold, reported as `BASELINE INTEGRITY: <path> — N line(s) …` with a per-file `diff` hint; deliberate divergence is silenced via `.ctsignore`, not thresholds. Regression-tested (case 6 + append-only negative control).
- **`.claude/scripts/cts-sync.sh` `merge_one()` raw-merge cross-check**: after each normalized 3-way merge, the same merge is rerun on raw blobs; if the raw merge finds more conflict regions than the normalized merge surfaced, the file is reported as `MERGE CROSS-CHECK: <path> — raw finds N … normalized surfaced M` with a repro hint. Closes the mechanism that anesthetized penny's data loss: when upstream's change to a region is formatting-only (e.g. the repo-wide `proseWrap: never` reflow), normalization makes base == upstream there, so a local deletion of that region silently wins instead of conflicting. Regression-tested (case 7, which reproduces the exact silent-drop and asserts the warning). Validated against real data: run on a scratch clone of penny's pre-repair HEAD, the guards reported `workflow.md` raw 10 vs normalized 3, `nx-generators.md` 4 vs 3, `distill-inbox` 3 vs 2, and baseline-integrity hits on `CLAUDE.md` (26 lines) / `workflow.md` (45 lines) / `tester.md` (20 lines) — matching the independent etalon investigation line for line, where the previous engine was silent.
...
- **`.claude/scripts/cts-sync.sh:315` `is_new_payload_collision()`**: its identical-content check used a raw `cmp`, inconsistent with the other 3 renormalize call sites — a brand-new prettier-handled payload path differing from the consumer's copy only by formatting was wrongly reported as a collision. Now routes through `norm_file` like the rest.
- **`.claude/skills/cts-update/SKILL.md` step 3**: the narration capture list and "Needs attention" grouping didn't mention the two new output line-kinds (`appended CTS-required lines...`, `new payload file already exists locally...`) — the second one increments `NEEDS_ATTENTION` exactly like the pre-existing categories but was silently never surfaced to the user. Both added.

### Fix Now — found only by live verification against real `penny` data, not the synthetic test suite

- **`.claude/scripts/cts-sync.sh` `append_missing_lines()`**: `echo "$line" >> "./$rel"` does not insert a newline before the first write if the target file doesn't already end with one — bash's `>>` just concatenates raw bytes. Penny's real `.prettierignore` has no trailing newline, so the append-merge path (the very fix for the bug that started this task) concatenated the first appended CTS line directly onto penny's last existing line (`.nx/self-healing# Vendored third-party skill...`), silently breaking that ignore entry. Every synthetic fixture in `tests/cts-sync.test.sh` was built via `{ echo ...; echo ...; } > file`, which always ends in a trailing newline — structurally incapable of exercising this path. Fixed by checking `[ -s "./$rel" ] && [ -n "$(tail -c 1 "./$rel")" ]` before the append loop and forcing a `printf '\n'` first when true. Regression-tested with a new fixture (case "1a-2") that deliberately omits the trailing newline, mirroring penny's real file exactly. Re-verified against a fresh scratch copy of the actual `../penny` repo (never the real directory itself) after the fix — confirmed clean. This is the clearest evidence this session's "test against real data, not just synthetic fixtures" verification step was load-bearing, not procedural.

### Changed

- **`CLAUDE.md`**: reverted to its original text — no host-repo-specific content, in any phrasing. An earlier pass this session added a clause here (even a self-detecting one, checking `cts-payload.txt`/`.cts-version`), but the user correctly flagged that **any** text describing claude-ts's own internal process, however conditionally phrased, is dead weight shipped into every consumer's `CLAUDE.md` forever for a condition that's false 100% of the time in their repo. The actual fix: `docs/KNOWLEDGE_INBOX.md` is guaranteed non-payload (enforced by `cts-sync.sh`'s `NEVER_PAYLOAD` hard-fail guard), so guidance about how to use that specific file belongs in that file's own header, co-located with what it governs — not forward-referenced from a shipped file. `CLAUDE.md`'s Knowledge-capture paragraph is now byte-identical to its pre-session text.
- **`docs/KNOWLEDGE_INBOX.md`**: header now carries the host-repo distill-direct policy (see above). Cleared 6 stale/resolved entries total across the session: one already captured as inline code comments (`merge_one`'s EXIT-trap discipline), one superseded by this changeset's own Bug A fix, one a self-corrected diagnostic mistake with no residual action, one distilled into the `cts-review-contribution` "Generalization integrity" checklist item, and two distilled into the new `rules/shell-scripting.md` (prettier CLI bin-shim realpath resolution; `objectWrap: preserve` default breaking naive fixture diffs).
- **`.claude/skills/cts-review-contribution/SKILL.md`**: added two checklist items. **Generalization integrity** — verify a contribution's generalization pass didn't merely find-replace a project-name literal (temp/lock-file prefixes, marker strings, log tags, cache-key segments, env var components) to a different single project's name instead of genuinely generalizing it; prompted by this session's `.claude/hooks/knowledge-capture-nudge.sh` marker-prefix bug (`penny-kc-nudge-` → `claude-ts-kc-nudge-`, still hardcoded to one project — the real uniqueness guarantee was Claude Code's own session_id all along). **Payload self-containment** — verify a payload-listed file change doesn't describe/assume the host repo's own identity or internal process, in any phrasing (including conditional); the structural counterpart to `tests/payload-sanity.test.sh` below, for cases too subtle for a phrase grep to catch.
- **`rules/shell-scripting.md`** (new file) + **`AGENTS.md`** on-demand rules index: three independent bash-scripting gotchas this session pointed at the same missing home — EXIT-trap-under-`set -u` (already in `cts-sync.sh` code comments), prettier CLI bin-shim realpath resolution, and prettier 3.5+'s `objectWrap: preserve` default breaking naive fixture diffs. Created once three occurrences made "no dedicated file yet" no longer the right call.
- **`tests/payload-sanity.test.sh`** (new file) — mechanical gate requested by the user after the `CLAUDE.md` host-leak mistake above: resolves every `cts-payload.txt` entry (respecting the `OWNER_ONLY_SKILLS` exclusion `cts-sync.sh` itself enforces) and greps for phrases assuming host-repo identity ("host repo", "template repo itself", "claude-ts itself", etc.). Ships with a small, justified allowlist for genuine two-way conditional rules that every reader needs (e.g. the pre-existing `CLAUDE.md`/`distill-inbox` sentences explaining that `docs/CLAUDE_TS_CHANGELOG.md` usage differs by repo type) — without it the gate would be permanently red on legitimate content and unusable in CI.

- **`.claude/skills/distill-inbox/SKILL.md`** (lines ~68, ~105): removed a reference to `project_host_repo_distill_direct` by name ("Per the `project_host_repo_distill_direct` convention...") — that string was the slug of a personal auto-memory file, not a checked-in repo convention any reader has access to, and was meaningless in shipped content. Both occurrences reworded to state the actual rule directly (`docs/CLAUDE_TS_CHANGELOG.md` only applies to consumer projects); the surrounding sentence already said this correctly, making the named-convention clause redundant as well as wrong.

### Minor items noted, not requiring a fix this session
