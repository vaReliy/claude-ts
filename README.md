...
3. **A payload file that diverged locally is only ever fast-forwarded, merged, or skipped — never silently overwritten.** A file is fast-forwarded (copied as-is) if the working copy still matches what was synced last time. If it diverged and upstream hasn't touched that file since, it's skipped and printed as `locally modified, not overwritten — diff manually: <path>` with a ready-to-run `git diff` command. If it diverged **and** upstream also changed it, `cts-sync.sh` runs a 3-way merge (base = the content at your old `.cts-version`) instead of skipping — see "Merge semantics" below. This is what makes it safe to edit payload files ahead of a `/cts-contribute` session without them being silently clobbered on the next update.
...
**Merge semantics.** Each payload file falls into one of four buckets on every `update`: **fast-forward** (only upstream changed it since your last sync — copied straight over, no output), **preserve** (only you changed it — skipped, printed as `locally modified, not overwritten`), **merge** (both of you changed it, in non-overlapping hunks — combined automatically with `git merge-file`, printed as `merged: <path>`), or **conflict** (both of you changed the same hunk — standard `<<<<<<<`/`=======`/`>>>>>>>` markers are left in the file for you to resolve by hand, printed as `CONFLICT: <path>`). Pass `--no-merge` to fall back to the old preserve-only behavior for every diverged file, merge attempts included.
